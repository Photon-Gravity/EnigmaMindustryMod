...
public class PolymorphRadar extends Radar {

	public PolymorphPowerStack consumed;

	public PolymorphRadar(String name) {
		super(name);
	}
	...
	public static Func<Building, Bar> makePowerBalance(){
		return entity -> new Bar(() ->
				...
				() -> ((IPolymorphUtilizer)entity).getModule() != null && ((IPolymorphUtilizer)entity).getModule().inSystem() ? ((IPolymorphUtilizer)entity).getModule().satisfaction(((PolymorphRadar)entity.block).consumed.type) : 0
		);
	}

	public class PolymorphRadarBuild extends RadarBuild implements IPolymorphUtilizer{
